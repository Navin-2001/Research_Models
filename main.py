...
async def load_models():
    ...
    try:
        ...
        logger.info("[Speech] Loading Whisper automatic speech recognition...")
        global whisper_asr
        whisper_asr = pipeline("automatic-speech-recognition", model="openai/whisper-tiny.en")
...
async def transcribe_audio(audio_file: UploadFile = File(...)):
    ...
    logger.info(f"Received audio for transcription: {audio_file.filename}")
    
    file_ext = os.path.splitext(audio_file.filename)[1] if audio_file.filename else '.m4a'
    temp_filename = f"temp_transcribe_{uuid.uuid4()}{file_ext}"
    ...
    try:
        ...
        # The transformers pipeline handles the ffmpeg loading automatically!
        result = whisper_asr(temp_filename)
        ...
        return {
            "success": False,
            "text": f"Sorry, could not transcribe audio: {str(e)}"
        }
    finally:
        # Cleanup temp file
